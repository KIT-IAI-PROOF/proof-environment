...
log_file_name = "proof_FileReaderLogger_" + str(options.local_block_id) + ".log"
logger = Logger('FileReaderLogger', handlers = [HandlerType.FILE], logging_dir=options.loggingDir, log_file_name = log_file_name, log_level=options.logLevel).get_logger()

class FileReader(BaseWrapper):
    """
    The FileReader module is a Python Wrapper for PROOF that allows users to read lines from a file and provide this line as output.
    ...
    def __init__(self, opt=options) -> None:
        ...
        super(FileReader, self).__init__(bwoptions=opt)
    ...
    async def init(self) -> None:
        ...
        try:
            ...
            await super(FileReader, self).init()
    ...
    async def step(self) -> None:
        ...
        try:
            ...
            await super(FileReader, self).step(BlockStatus.EXECUTION_STEP_FINISHED)
    ...
    async def finalize(self) -> None:
        ...
        await super(FileReader, self).finalize()


if __name__ == '__main__':
    try:
        asyncio.run(main(wrapper=FileReader()))
        logger.debug("The main method of the file_reader.py gets executed!")
